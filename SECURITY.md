...
| tmux | Pane registration and optional `send-keys` stay on hosts you already trust |
...
4. **Doorbell injection:** Opt-in terminal submit (`LETTERBOX_TMUX_SUBMIT=1`) can inject keystrokes into a live pane; leave it off unless you accept that risk. Use dedicated agent panes only.
5. **Shared hosts:** Do not expose Letterbox directories on untrusted multi-user systems without additional access control.
6. **SSH/headless:** SSH access to a tmux session does not move mailbox files. Put `LETTERBOX_DIR` on a filesystem every participant can actually see.
...
- Prefer live tmux terminal agents with durable inbox fallback.
...
- Prefer live pane self-registration over title/session-name matching when identities can collide.
