...
gate_name="test_private_vocabulary.sh"
...
# Token this edition's gate already forbids, built from parts so THIS file
# is not itself a residue hit. tmux matches the hyphenated private-path
# token, not a bare telegram.
tele_payload="$(printf 'residue shared''-brain here\n')"
